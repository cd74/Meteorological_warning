...
def train_and_test(encoder_forecaster, optimizer, criterion, lr_scheduler, batch_size, max_iterations, test_iteration_interval, test_and_save_checkpoint_iterations, folder_name, probToPixel=None):
    ...
    encoder_forecaster.load_state_dict(torch.load('models/encoder_forecaster_6000.pth'))
        ...
        if itera % test_iteration_interval == 0:
            ...
            with torch.no_grad():
                ...
                while not valid_hko_iter.use_up:
                    ...
                    for i in range(0,3):
                        for j in range(0,10):
                            cv2.imwrite('res/'+str(itera)+'_'+str(i)+'_'+str(j)+'.png',valid_batch[j][i][0]) 
                    valid_time += 1
                    ...
                    for i in range(0,3):
                        for j in range(0,10):
                            cv2.imwrite('res/'+str(itera)+'_'+str(i)+'_'+str(j+10)+'.png',out[j][i][0])
